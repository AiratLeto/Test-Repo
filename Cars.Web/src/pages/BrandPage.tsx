...
const BrandPage = () => {
    ...
    const onSave = async () => {
        ...
        try {
            if (editModel.id) {
                await BrandService.UpdateAsync(editModel.id, editModel as DictionaryItem);
            }
            else {
                await BrandService.CreateAsync(editModel as DictionaryItem);
            }
    ...
    return (
        <div>
            ...
            {editModel && <Modal onClose={() => setEditModel(undefined)}>
                <div>{editModel?.id ? 'Редактирование' : "Добавление"} бренда</div>
                <Input
                    label='Наименование'
